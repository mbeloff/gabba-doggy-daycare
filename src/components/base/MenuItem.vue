<template>
  <div @focus="openSub" @blur="mouseLeave" @mouseleave="mouseLeave" @click="clickMenu(item, item.link)" class="relative block lg:inline-block menu-item focus:outline-blue" tabindex="0">
    <button tabindex="0" class="block p-3 font-bold lg:inline-block focus:outline-none focus:text-blue-500 hover:text-blue-500">{{item.label}} <i v-if="item.sub" class="ml-2 fal fa-chevron-right fa-rotate-90"></i></button>
