<template>
  <div @focus="openSub" @blur="mouseLeave" @mouseleave="mouseLeave" @click="clickMenu(item, item.link)" class="relative block lg:inline-block menu-item focus:outline-blue" tabindex="0">
    <button tabindex="0" class="block px-3 py-3 lg:py-0 lg:inline-block focus:outline-none focus:text-blue-500 hover:text-blue-500 font-bold">{{item.label}} <i v-if="item.sub" class="ml-2 fal fa-chevron-right fa-rotate-90"></i></button>
