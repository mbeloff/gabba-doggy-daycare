    ...
    <form action="https://secure.petexec.net/remoteLogin.php" method="POST" class="flex flex-col px-3 pb-3 bg-white bg-right-bottom bg-no-repeat rounded shadow-md" id="petExecLogin">
      <div class="grid grid-cols-1 grid-rows-2 gap-0 sm:grid-cols-2 sm:grid-rows-1 sm:gap-4">
        <div class="mb-2">
          ...
          <input v-model="form.username" ref="name" class="w-full px-4 py-2 text-xl leading-tight text-gray-700 bg-white bg-opacity-50 border rounded shadow appearance-none focus:outline-none focus:shadow-outline focus:bg-opacity-75" name="username" id="username" type="text" placeholder="">
        </div>
        <div class="mb-2">
          <label class="block mt-3 mb-2 text-sm font-bold text-blue-500 uppercase" for="password">
            password
          </label>
          <input v-model="form.password" class="w-full px-4 py-2 text-xl leading-tight text-gray-700 bg-white bg-opacity-50 border rounded shadow appearance-none focus:outline-none focus:shadow-outline focus:bg-opacity-75" name="password" id="password" type="password" placeholder="">
        </div>
      </div>

      <div class="flex flex-row-reverse items-center col-span-2 justify-items-end">
...
secure.petexec.net/lostPassword.php" class="mr-3 text-sm text-pink-500 focus:outline-blue">forgot password?</a>
      </div>
