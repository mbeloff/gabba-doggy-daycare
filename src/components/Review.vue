  ...
  <div class="flex flex-col justify-between">
    <div class="m-2 px-4 py-3 bg-white rounded-lg shadow-lg border-gray-100 border" @click="expanded = !expanded">
      <div class="flex justify-between">
        <p class="font-bold text-sm text-blue-900">{{reviewData.name}}</p><span class="text-lg"><i class="fab fa-google text-white rounded-full bg-blue-500 p-1"></i></span>
      </div>

      <p ref="reviewBox" class="text-sm relative review-text" :class="{'truncate': !expanded}">{{reviewData.review}}  </p>
      <p class="text-xs text-right text-yellow-400 mt-2">
