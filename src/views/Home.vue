<template>
  <div>
    ...
    <div class="bg-gray-900 relative">
      <section-slider
        class="transition-opacity gallery duration-1000"
    ...
    <div class="section py-32 tile bg-gray-100 bg-opacity-30">
            ...
            >
              <template v-if="item.limit == getRegion() || item.limit == null">
                <div class="flex flex-col items-center gap-3">
                  ...
                  <p class="font-bold text-blue-700 self-center">{{item.title}} <span
                      class="text-blue-400 font-normal capitalize"
                      v-if="item.limit"
                    >- {{item.limit}} only</span> </p>
                </div>
                <p class="text-sm text-center">{{item.desc}}
                  <router-link
                    v-if="item.title == 'Grooming'"
                    class="link-pink"
                    :to="{name: item.title, params: {region: item.limit}}"
      ...
      <div class="mt-10 max-w-screen-lg mx-auto px-2 text-center flex flex-col sm:flex-row sm:flex-wrap  gap-x-2 gap-y-5">
        <router-link
          ...
          class="flex-grow btn-blue text-sm"
